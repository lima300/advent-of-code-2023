...
const rawInput: string = require("fs").readFileSync("input10.txt", "utf-8");
const diagram = rawInput.split(/\r?\n/).filter(Boolean);
const distances: Array<Array<null | number | string>> = new Array(
  diagram.length
)
  .fill(undefined)
...
const startPositions = getCardinalMovements(start).filter((position) =>
  getMoves(position).some(
    (movePosition) =>
      movePosition.row === start.row && movePosition.col === start.col
  )
);
...
console.log(
  "Part 2:",
  leftFloodFillResult.isOnBoundary
    ? rightFloodFillResult.count
    : leftFloodFillResult.count
);
